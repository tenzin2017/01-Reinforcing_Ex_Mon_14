...
possible_val =[]
venues.each do |hash|
  if hash[:wheelchair_accessible] == true && hash[:capacity] >= 150
    possible_val << hash[:address]
  end
end

puts "The possible venue are #{possible_val}.   There are #{possible_val.length} venues"
